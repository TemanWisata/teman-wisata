...
class Place(BaseModel):
    ...
    id: str | UUID4 = Field(description="Unique identifier for the place")
    place_id: int = Field(
        description="Unique identifier for the place in the database, from dataset",
    )
    place_name: str = Field(
        description="Name of the place, e.g., 'Museum Nasional', 'Taman Mini Indonesia Indah', etc.",
    )
    ...
    category: CategoryEnum = Field(
        default=CategoryEnum.BUDAYA,
        description="Category of the place, e.g., 'Budaya', 'Taman Hiburan', etc.",
    )
    province: str = Field(
        default="Jakarta",
        description="Province where the place is located, e.g., 'Jakarta', 'Bali', etc.",
    )
    price: float | None = Field(
        default=None,
        description="Price of the place, e.g., entry fee, ticket price, etc.",
    )
    rating: float = Field(
        default=0.0,
        description="Rating of the place, e.g., average user rating out of 5.",
    )
